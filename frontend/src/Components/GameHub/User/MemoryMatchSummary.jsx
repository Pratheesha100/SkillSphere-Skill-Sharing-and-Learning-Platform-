...
const MemoryMatchSummary = () => {
  ...
  const xpEarned = Math.round(results.score / 10); // 1 XP per 10 points
  const badges = [];
  
  if (results.score === 100) badges.push('Perfect Match');
  if (results.score >= 75) badges.push('Quick Thinker');
  if (results.score >= 50) badges.push('Getting Started');
...
export default MemoryMatchSummary;